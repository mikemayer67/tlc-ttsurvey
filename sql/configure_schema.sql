...
DROP PROCEDURE IF EXISTS tlc_tt_configure_schema;

-- start the procedure creation
CREATE PROCEDURE tlc_tt_configure_schema ()
...
-- This version adds a media registry table to the database
  CREATE TABLE tlc_tt_images (
  ...
  CREATE TABLE tlc_tt_image_derivatives (
    image_id    smallint UNSIGNED NOT NULL,
    usage_key   varchar(16) NOT NULL,
    size_key    smallint UNSIGNED NOT NULL COMMENT 'log_2(max dimensions)',
    width       smallint UNSIGNED NOT NULL,
    height      smallint UNSIGNED NOT NULL,
    PRIMARY KEY (image_id, size_key, usage_key),
    FOREIGN KEY (image_id) REFERENCES tlc_tt_images(image_id) ON UPDATE CASCADE ON DELETE CASCADE
  );

  CREATE TABLE tlc_tt_image_locks (
    usage_key varchar(150) PRIMARY KEY,
    image_id  smallint UNSIGNED NOT NULL,
    FOREIGN KEY (image_id) REFERENCES tlc_tt_images(image_id) ON UPDATE RESTRICT ON DELETE RESTRICT
  );

-- Add version 5 to the history and increment current version
  SET current_version = 5;
  INSERT INTO tlc_tt_version_history (version,description) values (current_version,'Add image library');
END IF;
...
CALL tlc_tt_configure_schema();

DROP PROCEDURE tlc_tt_configure_schema;
